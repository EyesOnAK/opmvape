...
local drawingactor = loadstring(downloadFile('opmsix/libraries/drawing.lua'), 'drawing')(...)
local redline = {Teams = {}}
...
local function warningRoutine(hash)
	local path = 'opmsix/profiles/agreementhash.txt'
...
if not select(1, ...) then
	if run_on_actor then
		...
		task.spawn(function()
			...
			local executionString = "loadfile('opmsix/main.lua')("..drawingactor..")"
