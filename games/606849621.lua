...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function() return game:HttpGet('https://raw.githubusercontent.com/EyesOnAK/opmvape/main/opmsix/'..select(1, path:gsub('opmsix/', '')), true) end)
...
local vm = loadstring(downloadFile('opmsix/libraries/vm.lua'), 'vm')()
